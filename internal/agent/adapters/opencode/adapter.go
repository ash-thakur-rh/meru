// Package opencode implements the meru agent adapter for OpenCode.
//
// OpenCode is started once in interactive TUI mode:
//
//	opencode [--model <model>]
//
// The process lives for the session lifetime under a PTY so the full TUI
// (colours, tool-use indicators, file edits) streams to the client in real
// time.  Callers can subscribe to raw PTY bytes via SubscribeRaw (for the
// bidirectional terminal WebSocket) or write text prompts via Send (for the
// programmatic API).
//
// xterm.js in the web UI renders the TUI output natively including all ANSI
// escape sequences.
package opencode
...
func (a *Adapter) Spawn(ctx context.Context, cfg agent.SpawnConfig) (agent.Session, error) {
	...
	slog.Debug("opencode starting interactive session", "workspace", cfg.Workspace, "model", cfg.Model)
	...
	if err := s.waitStartup(ctx); err != nil {
		...
		return nil, fmt.Errorf("opencode startup: %w", err)
	}

	s.setStatus(agent.StatusIdle)
	slog.Debug("opencode session ready", "session", s.id, "workspace", cfg.Workspace)
	return s, nil
}
...
// Send writes the prompt to the PTY stdin (which the TUI reads as user input)
// and streams raw PTY output as EventText events until 2 s of inactivity.
