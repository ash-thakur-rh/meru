...
func fakeOpenCodeScript(t *testing.T, body string) string {
	t.Helper()
	dir := t.TempDir()
...
func TestSpawn_PersistentProcess(t *testing.T) {
	...
	binDir := fakeOpenCodeScript(t, `echo "opencode ready"; cat`)
...
func TestSend_WritesToStdinAndStreams(t *testing.T) {
	...
	// Simulates the TUI reading stdin and echoing back (like a TUI input field).
	binDir := fakeOpenCodeScript(t, `
echo "ready"
...
`)
	...
	sess, err := New().Spawn(ctx, agent.SpawnConfig{Workspace: t.TempDir()})
	if err != nil {
		t.Fatal(err)
	}
	defer sess.Stop() //nolint:errcheck

	ch, err := sess.Send(ctx, "add validation")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(output.String(), "opencode: add validation") {
...
func TestSend_BusyRejectsSecondCall(t *testing.T) {
	...
	binDir := fakeOpenCodeScript(t, `echo "ready"; cat`)
	t.Setenv("PATH", binDir+":"+os.Getenv("PATH"))
...
func TestSafeBuffer(t *testing.T) {
	...
	if string(buf[:n]) != "hello world" {
		t.Errorf("Reader() = %q, want 'hello world'", string(buf[:n]))
	}
}
