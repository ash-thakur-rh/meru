...
func fakeGooseScript(t *testing.T, body string) string {
	t.Helper()
	dir := t.TempDir()
...
func TestSpawn_PersistentProcess(t *testing.T) {
	...
	binDir := fakeGooseScript(t, `echo "goose ready"; cat`)
...
`)
	...
	ch, err := sess.Send(ctx, "profile app")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(output.String(), "goose: profile app") {
...
func TestSend_BusyRejectsSecondCall(t *testing.T) {
	...
	binDir := fakeGooseScript(t, `echo "ready"; cat`)
	t.Setenv("PATH", binDir+":"+os.Getenv("PATH"))
