// Package aider implements the meru agent adapter for Aider.
//
// Aider is started once in interactive mode:
//
//	aider [--model <model>] --yes-always
//
// The process lives for the session lifetime under a PTY so the full terminal
// output — diffs, progress, file edits — streams to the client in real time.
// Callers can subscribe to raw PTY bytes via SubscribeRaw (for the
// bidirectional terminal WebSocket) or write text prompts via Send (for the
// programmatic API).
//
// Aider maintains its own conversation history internally across turns.
package aider
...
func (a *Adapter) Spawn(ctx context.Context, cfg agent.SpawnConfig) (agent.Session, error) {
	...
	slog.Debug("aider starting interactive session", "workspace", cfg.Workspace, "model", model)

	ptmx, err := pty.Start(cmd)
	if err != nil {
		slog.Error("failed to start aider", "error", err)
	...
	if err := s.waitStartup(ctx); err != nil {
		...
		return nil, fmt.Errorf("aider startup: %w", err)
	}

	s.setStatus(agent.StatusIdle)
	slog.Debug("aider session ready", "session", s.id, "workspace", cfg.Workspace)
	return s, nil
}
